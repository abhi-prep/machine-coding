...
public class TableCronFormatterService implements CronFormatterService {
    ...
    public TableCronFormatterService(CommandHandler commandHandler) {
    ...
    public String format(CronExpression cronExpression) {
        ...
        // Format year
        if(cronExpression.getYears() != null && !cronExpression.getYears().isEmpty()){
            output.append(formatField(FIELD_YEAR, cronExpression.getYears()));
        }

        // Format command
        output.append(String.format("%-" + FIELD_NAME_WIDTH + "s %s%n", FIELD_COMMAND, commandHandler.format(cronExpression.getCommand())));
